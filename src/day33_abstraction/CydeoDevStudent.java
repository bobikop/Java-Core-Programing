...
public class CydeoDevStudent extends Person {
    ...
    public CydeoDevStudent(String name, char gender, LocalDate dateOfBirth, String id, String batchName, int batchNumber) {
        super(name, gender, dateOfBirth);// constructor first must set then age, and then we can check age condition. Constructor call mist be first always!!!!

        if (getAge() < 18){  // if the age is set less than 18 --  // since instance variable is final (no setter), we can not add additional condition in setter. We use condition in constructor
            System.err.println("Cydeo Developer student must be at least 18 years old"); // If variable have a setter additional conditions will always need to be set in setter
            System.exit(1);
        }
    ...
    static{
        // static blocl for static variable
        programmingLanguage = "Java";
    }


    // fist generate getters then setters
    public String getId() {
        return id;
    }
